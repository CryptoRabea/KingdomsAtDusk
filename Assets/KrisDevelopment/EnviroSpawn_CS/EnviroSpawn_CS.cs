...
namespace KrisDevelopment.EnvSpawn
{
	...
	public class EnviroSpawn_CS : MonoBehaviour
	{
		...
		private static T[] FindObjects<T>(bool includeInactive) where T : Object
        {
# if UNITY_6000_0_OR_NEWER
		...
		{
			EnviroSpawn_CS[] objs = FindObjects<EnviroSpawn_CS>(false);
		...
		private void UpdateData()
		{
			...
			if (raycastPositions != null)
				...
				else
				{
					...
					Dictionary<Collider, bool> allColliders = new Dictionary<Collider, bool>();
					foreach (var obj in instanceObjects)
					{
						if (obj)
						{
							...
							{
								allColliders.Add(col, col.enabled);
								col.enabled = false;
							}
						}
					}
					allColliders.Keys.ToList().ForEach(x => x.enabled = false);

					for (int r = 0; r < population; r++)
					{
						RaycastHit hit;
						
						if (!Raycast(out hit, r) && scatterMode != 0)
						{
							raycastPositionsBeta[r] = transform.position + raycastPositions[r];
						}
						else if (bitmask != null && bitmask.EvaluateAll()){
							 //if any raycast is hitting a surface
							int attempts = 0;
							while (Raycast(out hit, r) ? ((avoidMask & 1 << hit.transform.gameObject.layer) == 1 << hit.transform.gameObject.layer) : (attempts++ < 10))
							{
					...
					//reset the colliders
					allColliders.Keys.ToList().ForEach(x => x.enabled = allColliders[x]);
				}
		}
