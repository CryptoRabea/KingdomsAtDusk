...
public class RTSCameraController : MonoBehaviour
{
    ...
    [Tooltip("Enable camera to follow terrain height, maintaining constant distance above ground")]
    public bool followTerrainHeight = false;
    [Tooltip("Layer mask for ground/terrain detection")]
    public LayerMask groundLayer = ~0; // Default to everything
    [Tooltip("Desired height above terrain when terrain following is enabled")]
    public float terrainHeightOffset = 20f;
    [Tooltip("How smoothly the camera adjusts to terrain height (higher = slower, smoother)")]
    public float terrainFollowSmoothness = 5f;
    [Tooltip("Maximum raycast distance for terrain detection")]
    public float terrainRaycastDistance = 500f;
    ...
    private void HandleTerrainFollowing()
    {
        if (!followTerrainHeight) return;

        // Cast a ray downward from the camera's XZ position to find terrain height
        Vector3 rayStart = new Vector3(transform.position.x, transform.position.y + terrainRaycastDistance * 0.5f, transform.position.z);
        Ray ray = new Ray(rayStart, Vector3.down);

        if (Physics.Raycast(ray, out RaycastHit hit, terrainRaycastDistance, groundLayer))
        {
            // Calculate desired Y position (terrain height + offset)
            float targetHeight = hit.point.y + terrainHeightOffset;

            // Smoothly interpolate to the target height
            float newY = Mathf.Lerp(transform.position.y, targetHeight, Time.deltaTime * terrainFollowSmoothness);
